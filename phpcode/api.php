...
if (isset($_POST['access_key']) && isset($_POST['add_expense'])) {
    ...
    if (!$input || empty($input['userId']) || empty($input['amount']) || empty($input['currency'])) {
        echo json_encode(["success" => false, "error" => "Invalid input"]);
        exit;
    }

    // Escape input values
    $userId = $db->escapeString($input['userId']);
    $amount = $db->escapeString($input['amount']);
    $currency = $db->escapeString($input['currency']);
    $description = isset($input['description']) ? $db->escapeString($input['description']) : '';
    $category = isset($input['category']) ? $db->escapeString($input['category']) : '';
    $date = isset($input['date']) ? $db->escapeString($input['date']) : '';

    // Insert expense
    $sql = "INSERT INTO expenses (firebase_id, amount, currency, description, category, date) VALUES ('$userId', '$amount', '$currency', '$description', '$category', '$date')";
    if ($db->sql($sql)) {
...
<?php
	...
	class Database {
		...
		private $db_name = 'expense-o'; // Change as required
